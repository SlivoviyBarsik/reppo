...
#SBATCH --array=0-26%26

env=(AcrobotSwingup AcrobotSwingupSparse BallInCup CartpoleBalance CartpoleBalanceSparse CartpoleSwingup CartpoleSwingupSparse CheetahRun FingerSpin FingerTurnEasy FingerTurnHard FishSwim HopperHop HopperStand PendulumSwingup ReacherEasy ReacherHard WalkerRun WalkerWalk WalkerStand HumanoidStand HumanoidWalk HumanoidRun HumanoidStand HumanoidWalk HumanoidRun)
hostname
...
python src/jaxrl/reppo.py --config-name=reppo \
    env=mjx_dmc_pert \
    env.name=${env[$((SLURM_ARRAY_TASK_ID%26))]} \
    seed=$RANDOM \
    tune=false \
    ...
    tags=[langevin,corr_pert,ent_var]
